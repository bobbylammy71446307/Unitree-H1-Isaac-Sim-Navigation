...
def generate_launch_description():
    ...
    map_dir = LaunchConfiguration("map",
        ...
        "maps", "humanoid_map.yaml"),
    )
    param_dir = LaunchConfiguration(
        "params_file",
        default=os.path.join(
            get_package_share_directory("my_slam"), 
            "config", "humanoid_nav.yaml"),
    )

    nav2_bringup_launch_dir = os.path.join(get_package_share_directory("nav2_bringup"), "launch")
    rviz_config_dir = os.path.join(get_package_share_directory("my_slam"), "rviz2", "humanoid.rviz")
    ...
    return LaunchDescription(
        [
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(os.path.join( get_package_share_directory("my_slam"), "launch", "rviz_launch.py")),
